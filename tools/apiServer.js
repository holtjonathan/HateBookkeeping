...
server.post('/chronicles/', function(req, res, next) {
	const error = validateChronicle(req.body);
...
server.post('/tribes/', function(req, res, next) {
	const error = validateTribe(req.body);
	if (error) {
		res.status(400).send(error);
	} else {
		req.body.slug = createSlug(req.body.name); // Generate a slug for new courses.
...
function validateChronicle(chronicle) {
	if (!chronicle.name) return 'Name is required.';
	//if (!chronicle.adminId) return 'Admin is required.';
	return '';
}

function validateScenario(scenario) {
	if (!scenario.name) return 'Name is required.';
	return '';
}

function validateTribe(tribe) {
	if (!tribe.name) return 'Name is required.';
	return '';
}
